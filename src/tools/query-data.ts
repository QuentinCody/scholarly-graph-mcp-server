...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
    ...
    server.registerTool(
        "scholarly_graph_query_data",
        {
            title: "Query Staged Scholarly Graph Data",
            description:
                "Query staged scholarly graph data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
